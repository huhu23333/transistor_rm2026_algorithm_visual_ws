...
public:
    ...
    // 子 EKF 的内部状态
    using PosState = PosEKF::MatrixX1; // [xc, vxc, yc, vyc, zc, vzc]
    using GeoState = GeoEKF::MatrixX1; // [yaw, vyaw, r, dz]

    // 对外 10 维完整状态：
    // [0] xc, [1] vxc, [2] yc, [3] vyc,
    // [4] zc, [5] vzc, [6] yaw, [7] vyaw,
    // [8] r,  [9] dz
    ...
    Eigen::Vector3d predictBestArmorPosition(double t_ahead, double switch_thres_rad = 0.2) const;
    
    // 4 装甲跳变处理（在 predict/update 前调用）
    void handleArmorJump(double measured_yaw,
                         const Eigen::Vector3d &measured_pos);

    // 由状态反解“当前这块装甲”的三维位置（便于匹配/调试）
    static Eigen::Vector3d armorPositionFromState(const State& x,
                                                  int offset_sign = OFFSET_SIGN);

    const State &state() const { return x_full_; }
    TrackState trackState() const { return state_flag_; }

private:
    // 根据子 EKF 状态更新完整的 10 维 x_full_
    void syncFullFromSub();

private:
    // 两个内部 EKF
    std::unique_ptr<PosEKF> pos_ekf_;
    std::unique_ptr<GeoEKF> geo_ekf_;

    EKFParams params_;

    PosState x_pos_ {};   // [xc, vxc, yc, vyc, zc, vzc]
    GeoState x_geo_ {};   // [yaw, vyaw, r, dz]
    State    x_full_{};   // 对外暴露的 10 维状态

    double      dt_ {};           // 时间步长
    MotionModel model_;           // 位置 EKF 使用的运动模型
    TrackState  state_flag_;      // 状态机
