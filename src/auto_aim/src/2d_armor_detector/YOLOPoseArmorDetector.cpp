...
std::vector<Armor> YOLOPoseArmorDetector::detectArmors(const cv::Mat& frame, bool block) {
    std::vector<Armor> result;

    std::vector<DetectionResult> yolo_pose_detection_results = shm_python_yolo_pose -> processImage(frame, block);
    ...
    for (DetectionResult& yolo_pose_detection_result : yolo_pose_detection_results) {
        if (yolo_pose_detection_result.confidence > 0.0) {
            ...
            RCLCPP_INFO(node->get_logger(), "scaled_yolo_data: %f, %f, %f, %f, %f, %f, %f, %f",
            ...
            cv::Size2f leftLightBar_size(leftLightBar_length / 8.0, leftLightBar_length);
            cv::Size2f rightLightBar_size(rightLightBar_length / 8.0, rightLightBar_length);
            ...
            result.emplace_back(leftLightBar, rightLightBar, config_file_ptr, node);
        }
    }
