...
EKFTracker::EKFTracker(double dt, const EKFParams& params) : dt_(dt), state(LOST)
{
...
void EKFTracker::reset(const Measurement& z) {
    state = DETECTING;
...
EKFTracker::State EKFTracker::getTargetState() const { return ekf_->getState(); }

Eigen::Vector3d EKFTracker::getArmorPosition() const {
...
void EKFTracker::setParams(const EKFParams& params) {
    RobotEKF::UpdateQFunc update_Q = [this, params]()
    {
        Eigen::Matrix<double, N_x, N_x> Q = Eigen::Matrix<double, N_x, N_x>::Zero();
        double t = this->dt_;
