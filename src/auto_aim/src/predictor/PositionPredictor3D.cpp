// PositionPredictor3D.cpp
#include "predictor/PositionPredictor3D.h"


PositionPredictor3D::PositionPredictor3D(int max_history) 
...
void PositionPredictor3D::addPoint(const cv::Point3f& point) {
...
void PositionPredictor3D::fitLinear(int steps) {
    if (history_.empty()) {
    ...
    // 提取最新的实际步数个点
    std::vector<cv::Point3f> recent_points(
...
void PositionPredictor3D::fitQuadratic(int steps) {
    if (history_.empty()) {
    ...
    try {
        ...
        quadratic_fit_z_.a0 = linear_fit_z_.b;
        quadratic_fit_z_.a1 = linear_fit_z_.a;
        quadratic_fit_z_.a2 = 0.0;
        quadratic_fit_z_.mse = linear_fit_z_.mse;
...
void PositionPredictor3D::fitFourier(int steps, int fourier_order) {
    if (history_.empty()) {
    ...
    std::vector<double> res_z = linear_fit_z_.residuals;
    ...
    fourier_fit_.beta_z = fitFourierSeries(res_z, T, fourier_order);
    ...
    for (int t = 0; t < actual_steps; t++) {
        ...
        double pred_z = linear_fit_z_.a * t + linear_fit_z_.b + 
                       fourierSeries(t, fourier_fit_.beta_z, T, fourier_order);
...
std::vector<cv::Point3f> PositionPredictor3D::predictLinear(int pred_length) const {
    ...
    for (int i = 0; i < pred_length; i++) {
        ...
        float z = linear_fit_z_.a * t + linear_fit_z_.b;
        predictions.push_back(cv::Point3f(x, y, z));
    }
...
// 新增：二次曲线预测函数实现
std::vector<cv::Point3f> PositionPredictor3D::predictQuadratic(int pred_length) const {
    std::vector<cv::Point3f> predictions;
    ...
    for (int i = 0; i < pred_length; i++) {
        ...
        float z = quadratic_fit_z_.a0 + quadratic_fit_z_.a1 * t + quadratic_fit_z_.a2 * t * t;
        predictions.push_back(cv::Point3f(x, y, z));
    }
...
std::vector<cv::Point3f> PositionPredictor3D::predictFourier(int pred_length) const {
    std::vector<cv::Point3f> predictions;
    ...
    for (int i = 0; i < pred_length; i++) {
        ...
        float z = linear_fit_z_.a * t + linear_fit_z_.b + 
                 fourierSeries(t, fourier_fit_.beta_z, T, N);
        predictions.push_back(cv::Point3f(x, y, z));
    }
...
double PositionPredictor3D::getLastMSE() const {
    return last_mse_;
}

void PositionPredictor3D::clearHistory() {
    history_.clear();
    point_count_ = 0;
    linear_fitted_ = false;
...
int PositionPredictor3D::getPointCount() const {
    return point_count_;
}
...
cv::Vec3f PositionPredictor3D::getLinearVelocity() const {
    ...
    return cv::Vec3f(
        ...
        static_cast<float>(linear_fit_z_.a)
    );
}
...
PositionPredictor3D::QuadraticFitResult PositionPredictor3D::fitQuadraticComponent(
    const std::vector<float>& y, int point_count, int steps) const 
{
...
double PositionPredictor3D::fourierSeries(double t, const std::vector<double>& beta, int T, int N) const {
